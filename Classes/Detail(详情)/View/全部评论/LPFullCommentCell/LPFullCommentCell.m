...
- (void)setFullCommentFrame:(LPFullCommentFrame *)fullCommentFrame
{
    ...
    [self.iconView sd_setImageWithURL:[NSURL URLWithString:comment.userIcon] placeholderImage:[UIImage oddityImage:@"评论用户占位图"]];
    ...
    self.commentLabel.attributedText = self.fullCommentFrame.attributeCommentText;
    ...
    if ([comment.isPraiseFlag isEqualToString:@"1"]) {
        [self.upButton setBackgroundImage:[UIImage oddityImage:@"详情页已点赞"] forState:UIControlStateNormal];
    } else {
        [self.upButton setBackgroundImage:[UIImage oddityImage:@"详情页未点赞"] forState:UIControlStateNormal];
    }
