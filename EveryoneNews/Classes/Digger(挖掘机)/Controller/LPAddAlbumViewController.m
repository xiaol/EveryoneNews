...
static NSString * const CoverCellReuseId = @"coverCell";

NSString * const AlbumDidCreatedSuccessNotification = @"com.everyonenews.createalbumsuccessfully";

static const NSInteger totalCoverNums = 8;

@interface LPAddAlbumViewController () <UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation LPAddAlbumViewController
...
- (void)changeCoverImage {
    if (self.selectedIndexPath.row == self.covers.count - 1) {
...
- (void)createBtnClick {
    ...
    if (!title.length) {
        ...
        if (self.selectedIndexPath.item < totalCoverNums - 1) {
            [Album albumWithID:self.addID title:title subtitle:subtitle thumbnailImage:cover.image inManagedObjectContext:cdh.context];
        } else {
            [Album albumWithID:self.addID title:title subtitle:subtitle photo:cover.image inManagedObjectContext:cdh.context];
        }
        // 发通知?
        [noteCenter postNotificationName:AlbumDidCreatedSuccessNotification object:nil];
...
- (void)textFieldsChangeText {
    self.createBtn.enabled = (self.titleField.text.length && self.selectedIndexPath);
}
