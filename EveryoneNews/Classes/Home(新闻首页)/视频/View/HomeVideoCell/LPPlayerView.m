...
- (void)loadMediaOptions {
    ...
    if (group) {
        ...
        [self.playerItem selectMediaOption:options.firstObject inMediaSelectionGroup:group];
    }
}
            ...
            if (self.isFullScreen && self.hasPreviewView) {
                ...
                AVAssetImageGeneratorCompletionHandler handler = ^(CMTime requestedTime, CGImageRef im, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *error) {
                    if (result != AVAssetImageGeneratorSucceeded) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [controlView lp_playerDraggedTime:dragedSeconds sliderImage:self.thumbImg ? : LPPlayerImage(@"video_loading_bgView")];
                        });
                    } else {
