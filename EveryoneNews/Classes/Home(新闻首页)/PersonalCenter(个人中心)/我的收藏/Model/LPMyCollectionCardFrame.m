...
- (void)setCard:(LPMyCollectionCard *)card {
    ...
    if (card.rtype == videoNewsType) {
        ...
        NSMutableAttributedString *singleAttrStr =  [singleStr attributedStringWithFont:[UIFont systemFontOfSize:titleFontSize] lineSpacing:lineSpacing];
        CGRect singleRect = [singleAttrStr boundingRectWithSize:CGSizeMake(titleW, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];
        CGFloat singletitleH = singleRect.size.height;
        if (titleH > 3 * (singletitleH +2)) {
            titleH = 3 * (singletitleH +2);
        }
        ...
        CGFloat sourceSiteNameY = CGRectGetMaxY(_videoTitleLabelFrame) + sourcePaddingTop1;
        _videoSourceLabelFrame = CGRectMake(PaddingHorizontal, sourceSiteNameY, titleW, sourceSiteNameH);
        CGFloat videoImageSeperatorLineY = 0.f;
        
        if ( (titleH ) + sourceSiteNameH + sourcePaddingTop1 > imageH) {
            videoImageSeperatorLineY = CGRectGetMaxY(_videoSourceLabelFrame) + paddingBottom;
            _videoCommentsCountLabelFrame = CGRectMake(ScreenWidth - commentLabelW - commentLabelPaddingRight, sourceSiteNameY, commentLabelW, sourceSiteNameH);
            
        } else {
            videoImageSeperatorLineY = CGRectGetMaxY(_videoImageViewFrame)+ paddingBottom;
            _videoCommentsCountLabelFrame = CGRectMake(ScreenWidth - commentLabelW - commentLabelPaddingRight - imageW - PaddingHorizontal, sourceSiteNameY, commentLabelW, sourceSiteNameH);
        }
        ...
        _videoSeperatorLineFrame = CGRectMake(0, videoImageSeperatorLineY, ScreenWidth, 0.5);
