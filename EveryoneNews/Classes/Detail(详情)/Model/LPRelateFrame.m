...
- (void)setRelatePoint:(LPRelatePoint *)relatePoint {
   ...
   _relatePoint = relatePoint;
    ...
    if (_relatePoint.updateTime.length > 5) {
        ...
        _yearPoint = CGPointMake(BodyPadding + circleHeight / 2, padding1 + yearSize.height / 2);
        _monthDayPoint = CGPointMake(BodyPadding + circleHeight / 2 , yearSize.height + padding1 * 2 + monthSize.height / 2);
        ...
        _monthDayPoint = CGPointMake(BodyPadding + circleHeight / 2,  padding1 + monthSize.height / 2);
    ...
    if (_relatePoint.imgUrl.length > 0 && [_relatePoint.imgUrl rangeOfString:@","].location == NSNotFound) {
        ...
        _seperatorViewF = CGRectMake(titleX, CGRectGetMaxY(_imageViewF) + padding1, ScreenWidth - titleX - BodyPadding * 2 - padding2 - circleWidth, 0.5);
        ...
        _seperatorViewF = CGRectMake(titleX, CGRectGetMaxY(_sourceSiteF) + padding1, ScreenWidth - BodyPadding * 2 - titleX - padding2 - circleWidth, 0.5);
        
    }
    _cellHeight = CGRectGetMaxY(_seperatorViewF);
    _cellViewF = CGRectMake(BodyPadding + circleWidth + padding2, 0,ScreenWidth - BodyPadding * 2 - titleX, _cellHeight);
}
